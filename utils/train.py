...
def get_train_loader(tgt_context_sets: list, batch_size: int = 64) -> DataLoader:
    ...
    train_data = TorchTargetContextDataset(x_train_tensor, y_train_tensor)
...
def train_model(
    ...
    for epoch in tqdm(range(epochs), disable=not verbose):
        ...
        if verbose:
            if len(sectors) != 0:
                evaluate_embeddings(model.embeddings.weight.detach().cpu().numpy(), sectors=sectors, top_k_accuracy=True, smote=True, n_splits=5)
            print(f"Epoch {epoch+1}: Loss = {epoch_loss:.4f}", end="\r")
...
class TorchTargetContextDataset(Dataset):
