...
class StockData:
    ...
    def _get_data(self):
        daily_returns_df = pd.read_csv(self.daily_returns_path, index_col="Date").drop(columns="Unnamed: 0")
    ...
    def _get_extras(self, misc_include=False):
        ...
        for ticker in tickers:
            self.sectors.append(self.extras_df[self.extras_df["ticker"] == ticker].sector.values[0])
            self.industries.append(self.extras_df[self.extras_df.ticker == ticker].industry.values[0])
    ...
    def __repr__(self):
        self.num_stocks = len(set(self.tickers))
        self.num_sectors = len(set(self.sectors))

        representation = f"""\
StockData(
    directory={self.daily_returns_path},
